...
func run(lab:Node3D) -> void:
	...
	for tick in 160:
		await frames(1)
		var query:=PhysicsShapeQueryParameters3D.new();query.shape=p.collider.shape;query.transform=p.collider.global_transform;query.collision_mask=1;query.exclude=[p.get_rid()]
		for hit in p.get_world_3d().direct_space_state.intersect_shape(query):
			if hit.collider==blocker:crossed_solid=true
	check(not crossed_solid,"reeling routes around a blocking slab without teleporting the body through it")
	...
	check(dummy.knocked_out and dummy.last_hit_kind=="punch" and dummy.respawn>0,"punch knocks out the practice dummy with recovery rather than killing it")
	...
	check(p.avatar.shoulder_position(0).y>p.position.y+1.0,"launch arms remain attached to the visible unicycle shoulders")
	check(p.hands[0].fist.position.y-p.position.y>1.0 and p.hands[0].fist.position.y-p.position.y<1.6,"third-person charge fists stay beside the unicycle torso")
