...
func run(lab: Node3D) -> void:
	...
	check(p.avatar.skeleton.get_bone_count()==12 and p.avatar.clips.size()>=6,"courier has a 12-bone skeleton and six imported animation clips")
	...
	check(p.avatar.ball_shell.visible and not p.avatar.body.visible,"third-person slingshot has a compact ball silhouette")
	p._unhandled_input(key)
