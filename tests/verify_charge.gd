...
func run(lab: Node3D) -> void:
	...
	check(avatar.current_clip=="Charge","moving charge uses the wind-up while the independent wheel keeps rolling")
	...
	c.cancel()
	check(avatar.skeleton.get_bone_count()==9,"runtime courier imports nine unicycle joints")
	for clip in ["Idle","Walk","Air","Charge","Punch","Land"]:
		check(avatar.clips.has(clip) and avatar.animator.get_animation(avatar.clips[clip]).get_track_count()>=3,"imported skeletal clip has animation tracks: "+clip)
	var wheel:int=avatar.skeleton.find_bone("wheel")
	p.velocity=Vector3(0,0,-8);avatar.pose(p,.05)
	var rotation:Quaternion=avatar.skeleton.get_bone_pose_rotation(wheel)
	avatar.pose(p,.05)
	check(rotation.angle_to(avatar.skeleton.get_bone_pose_rotation(wheel))>.5,"rolling distance drives wheel rotation independently of clips")
	p.crouched=true;avatar.pose(p,.2)
	check(avatar.body.scale.is_equal_approx(Vector3.ONE) and avatar.skeleton.find_bone("foot_L")==-1,"braking compresses suspension without squashing or leg bones")
