...
func run(lab: Node3D) -> void:
	...
	check(p.combat.charging and not p.combat.active,"70 ms left-right chord starts charging")
	var origin:Vector3=p.position
	await frames(135)
	p.handle_glove_button(0,false,2300)
	await frames(55)
	...
	check(p.combat.charging,"right-left chord works within 120 ms")
	p.handle_glove_button(1,false,2120)
	p.cancel_hands()
