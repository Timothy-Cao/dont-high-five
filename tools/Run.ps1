...
try {
    ...
    if ($Mode -eq 'Verify') {
        foreach ($suite in @('verify','metrics','polish','arena-verify','expansion-verify','combat-power','settings-verify','charge-verify','recoil-verify','density-verify','fixed-verify','pilot-verify','wrap-verify','carry-night-verify','orbital-verify','crowd-verify','workshop-verify','reveal-verify','minimap-verify','refinement-verify','workshop-gameplay-verify')) {
            Invoke-CheckedEngine ($suite + '.log') @('--headless','--',('--' + $suite))
        }
        Write-Host 'All twenty-one suites passed. Reports and logs are in .local/.'
        exit 0
    }
