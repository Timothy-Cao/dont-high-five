...
$outputPath = Join-Path (Resolve-Path -LiteralPath $OutputDirectory).Path 'ParcelPop-standalone.zip'
$files = [System.Collections.Generic.List[System.IO.FileInfo]]::new()
foreach ($name in @('project.godot','README.md','Play.cmd','Edit.cmd','Verify.cmd','.gitignore','.gitattributes')) {
    $files.Add((Get-Item -LiteralPath (Join-Path $projectRoot $name) -Force))
}
foreach ($folder in @('scripts','scenes','shaders','assets','tests','art','docs','licenses')) {
...
try {
    foreach ($file in $files) {
        ...
        [System.IO.Compression.ZipFileExtensions]::CreateEntryFromFile($zip, $file.FullName, ('parcel-pop/' + $relative), [System.IO.Compression.CompressionLevel]::Optimal) | Out-Null
    }
} finally {
    $zip.Dispose()
    $stream.Dispose()
