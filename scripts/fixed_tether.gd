...
func attach(player: CharacterBody3D,index: int) -> void:
	...
	h.rest=maxf(0.5,player.arm_length(h,player.chest()))
...
func constrain(player: CharacterBody3D,dt: float) -> void:
	...
	var tail:float=player.arm_tail(h)
	var requested:float=maxf(tail+0.5,move_toward(old_length,h.spool_target,winch_speed*dt))
	pending_contribution=Vector3.ZERO
	var result:=integrate(player.chest()-player.arm_anchor(h),player.velocity,maxf(0.5,requested-tail),dt)
	var destination:Vector3=player.arm_anchor(h)+result.offset-Vector3.UP*1.15
	h.rest=requested
...
func after_move(player: CharacterBody3D) -> void:
	...
	if player.get_slide_collision_count()==0:
		...
		var h:Dictionary=player.hands[active_hand]
		h.rest=maxf(h.rest,player.arm_length(h,player.chest()))
