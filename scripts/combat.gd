...
func charge_fraction() -> float:
	return clampf(charge_time/CHARGE_SECONDS,0,1)

func start_charge() -> bool:
	if player.arms_suppressed() or not player.punch_enabled or player.stone or player.held("brake") or player.held("anchor") or player.hand_recovery>0 or active or charging: return false
	player.cancel_hands()
...
func impact(index: int,contact: Dictionary) -> void:
	...
	if normal.length()>0.5 and distance<RECOIL_RADIUS and body is CollisionObject3D and (body.collision_layer&1)!=0:
...
func tick(dt: float) -> void:
	...
	for i in 2:
		...
		q.collision_mask=11;q.exclude=[player.get_rid()];q.margin=0.005
		...
		var ray_query:=PhysicsRayQueryParameters3D.create(previous,previous+direction*(step+shape.radius),11,[player.get_rid()])
